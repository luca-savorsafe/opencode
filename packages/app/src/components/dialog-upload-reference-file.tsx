...
interface DialogUploadReferenceFile {
  title?: string
  multiple?: boolean
...
export function DialogUploadReferenceFile(props: { projectRoot: string }) {
  ...
  function uploadFiles() {
    ...
    if (client) {
      ...
      client.post({
        url: `/upload-reference/${projectRoot}`,
        body: formData,
        bodySerializer: (body) => {
          return body
        },
    ...
    // sdk.client.uploadReference({ directory: root() }, { files: filePaths }).then((x) => {
    //   if (x.data) {
    //     showToast({
    //       description: "上传成功",
    //       variant: "success",
  ...
  return (
    <Dialog title="Upload reference file">
      <div class="p-4">
        <input
          type="file"
          ref={fileInputRef}
